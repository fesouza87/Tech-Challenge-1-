...
|---|---|---|
| Analise multimodal de texto, audio e video | Parcial | Pipelines e rotas em `src/api/routes_pipelines.py` e `src/pipelines/` |
| Analise de video clinico | Parcial | Validado com video sintetico em `data/synthetic/media/fisioterapia_demo.mp4`, leitura real de frames, relatorios em `reports/video/` e inferencia com pose/objeto |
...
| Analise postural com OpenPose | Parcial | Suporte opcional a OpenPose quando configurado; fallback com MediaPipe validado localmente no pipeline de video |
...
| Relatorio tecnico | Parcial | Documento base em `RELATORIO_TECNICO.md` ja inclui fluxo, tecnicas e resultados de validacao sintetica; faltam resultados finais da entrega |
