...
const DEMO_AUDIO_TRANSCRIPT = "Patient has coughing, mild breathing, and reports fatigue during the consultation.";
...
const elements = {
  ...
  statHighAlerts: document.getElementById("stat-high-alerts"),
...
async function loadOverview() {
  ...
  elements.statPatients.textContent = overview.stats.patient_count;
  elements.statEvents.textContent = overview.stats.event_count;
  elements.statAlerts.textContent = overview.stats.alert_count;
  elements.statHighAlerts.textContent = overview.stats.high_alert_count;
...
function prependLiveAlert(alert) {
  const current = elements.liveAlerts.innerHTML;
  const html = `
    <div class="list-item">
      <div class="list-title">
      ...
      <div class="list-meta">${alert.patient_id} | ${alert.modality}</div>
      <div class="list-meta">${alert.message}</div>
      <div class="list-meta">${formatDate(alert.created_at)}</div>
    </div>
  `;
  elements.liveAlerts.innerHTML = html + current;
}
...
function buildAudioFormData() {
  ...
  if (!elements.audioTranscript.value.trim()) {
    formData.delete("transcript");
  } else {
    formData.set("transcript", elements.audioTranscript.value.trim());
  }
...
async function processVitalImport(payload, statusMessage) {
  ...
  try {
    const response = await fetch("/api/dashboard/demo-audio");
    if (!response.ok) {
      throw new Error(await extractErrorMessage(response));
    }

    const blob = await response.blob();
    const file = new File([blob], "speech_demo_en.wav", { type: blob.type || "audio/wav" });
    const transfer = new DataTransfer();
    transfer.items.add(file);
    elements.audioFile.files = transfer.files;
    elements.audioPatientId.value = "PAZDEMO";
    elements.audioLanguage.value = "en-US";
    elements.audioTimestamp.value = defaultTimestampValue();
    elements.audioTranscript.value = DEMO_AUDIO_TRANSCRIPT;
...
async function handleVitalImport(event) {
  event.preventDefault();
  await processVitalImport(buildVitalImportPayload(), "Importando sinais vitais...", "info");
}
...
elements.audioTimestamp.value = defaultTimestampValue();
setUploadStatus("Envie um arquivo ou informe apenas a transcricao para testar o pipeline.", "info");
